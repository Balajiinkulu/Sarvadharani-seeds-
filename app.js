    ...
    function renderInvoicesList() {
        ...
        const all = transactions
            // Cancelled placeholders carry no value and nothing is owed on
            // them — listing them here would just add zero-value noise to a
            // screen that exists to show what still needs paying.
            .filter(t => t.type === kind && !t.cancelled && inRange(t.date, range))
            .map(t => {
    ...
    // Maps a voucher-number prefix back to the voucher type it belongs to,
    // so a filler voucher is created as the right kind and shows up in the
    // right places.
    const GAP_PREFIX_TYPE = {
        INV: 'Sales', PUR: 'Purchase', RPUR: 'Purchase',
        PYMT: 'Payment', RCPT: 'Receipt', JRNL: 'Journal',
        DCHL: 'DeliveryNote', CHL: 'DeliveryNote'
    };

    // Fills every gap in a series with a zero-value voucher marked
    // "Cancelled". Nothing existing is touched — no renumbering, no
    // re-linking — so invoices already given out and payments already
    // recorded stay exactly as they are. The point is only that each
    // missing number has a written explanation if it's ever queried.
    async function fillGapsWithCancelled(fy, prefix, missingCsv) {
        const missing = String(missingCsv).split(',').map(n => parseInt(n, 10)).filter(n => n > 0);
        if (!missing.length) return;
        const type = GAP_PREFIX_TYPE[prefix] || 'Sales';
        const label = missing.map(n => String(n).padStart(4, '0')).join(', ');

        const ok = await confirmAsync(
            `Create ${missing.length} cancelled voucher${missing.length === 1 ? '' : 's'} in ` +
            `SDS/${fy}/${prefix} for the missing number${missing.length === 1 ? '' : 's'}:\n\n${label}\n\n` +
            `Each will be a zero-value ${type} voucher marked CANCELLED. Nothing already ` +
            `posted is changed or renumbered.`);
        if (!ok) return;

        // Date them within the right financial year rather than today, so a
        // 25-26 gap doesn't appear as a 26-27 voucher and create a new
        // out-of-order problem while fixing this one.
        const startYear = 2000 + parseInt(String(fy).split('-')[0], 10);
        const fyDate = `${startYear}-04-01`;

        missing.forEach(n => {
            transactions.push({
                id: newId(transactions),
                invNo: `SDS/${fy}/${prefix}/${String(n).padStart(4, '0')}`,
                date: fyDate,
                type: type,
                cancelled: true,
                taxType: 'EXEMPT',
                partyId: null,
                partyName: 'CANCELLED — number not used',
                items: [],
                taxable: 0,
                totalTax: 0,
                grandTotal: 0,
                narration: 'Cancelled: this number was generated by the system but no voucher was issued against it.'
            });
        });

        localStorage.setItem('tally_mob_db', JSON.stringify(transactions));
        syncCloud();
        logAudit('Created', { invNo: `${missing.length} cancelled voucher(s)`, type: type, grandTotal: 0 },
                 `Filled gaps in SDS/${fy}/${prefix}: ${label}`);
        renderInvoiceGapCheck();
        render();
        alert(`${missing.length} cancelled voucher${missing.length === 1 ? '' : 's'} created. This series now has no gaps.`);
    }
    ...
    function renderInvoiceGapCheck() {
        ...
        const rowsHtml = seriesKeys.map(key => {
            ...
            return `<tr>
                ...
                <td>${missingLabel}</td>
                <td>${statusHtml}${missing.length ? `<div style="margin-top:6px;"><button type="button" class="btn-inline" style="width:auto; padding:4px 10px; font-size:0.72rem;" onclick="fillGapsWithCancelled('${escapeHtml(s.fy)}','${escapeHtml(s.prefix)}','${missing.join(',')}')">Fill with cancelled</button></div>` : ''}</td>
            </tr>`;
        }).join('');
